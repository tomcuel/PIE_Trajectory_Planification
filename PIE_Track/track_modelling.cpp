...
int main ()
{
    ...
    // Export the graph with the shortest path
    ...
    // adding all the points except the last one that is reversed
    for (int i = 0; i < shortest_path.size()-1; i++){
        path_points.push_back(shortest_path[i]->points[0]);
    }
    // adding the last element that is reversed
    path_points.push_back(shortest_path[shortest_path.size()-1]->points[1]);
    path_points.push_back(shortest_path[shortest_path.size()-1]->points[0]);
    ...
    path_elevation_file << "elevation,utm_x,utm_y,distance from start" << std::endl;
    ...
    // printing the lenth of the path
    double total_lap_length = 0;
    for (Arc* arc : shortest_path){
        total_lap_length += arc->length();
    }
    // link the last and first points
    total_lap_length += sqrt((path_points[path_points.size()-1].x - path_points[0].x)*(path_points[path_points.size()-1].x - path_points[0].x) + (path_points[path_points.size()-1].y - path_points[0].y)*(path_points[path_points.size()-1].y - path_points[0].y));
    std::cout << "Total lap length : " << total_lap_length << " meters" << std::endl;
